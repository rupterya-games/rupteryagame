...
import { abilities, classes, equipment, huntCreatures, huntRegions, kingdoms, rollFiordevalleEncounter, sharedAbilities } from "@/lib/catalog";
...
export default function HomePage() {
  ...
  const regionCreatures = huntCreatures.filter((creature) => creature.regionId === region.id);
  ...
  const startHunt = (creatureId: string) => {
    if (!selected || selected.vitals.hpCurrent <= 0) { setMessage("O personagem está sem HP. A cura/estalagem será o próximo serviço do mundo."); return; }
    const creature = huntCreatures.find((entry) => entry.id === creatureId);
    if (!creature) return;
    ...
    setMessage(`Encontro iniciado em ${region.name}.`);
  };
