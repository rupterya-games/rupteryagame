...
export function createHuntBattle(input: { regionId: string; player: HuntCombatant; creatures: HuntCreatureDefinition[]; companion?: HuntCompanion | null; itemMemories?: Record<string, number> }): HuntBattleState {
  ...
  const enemies: HuntCombatant[] = creatures.map((creature, index) => {
    ...
    return { id: `${creature.id}-${index}`, name: creature.name, portraitPath: creature.portraitPath, hpCurrent: hpMax, hpMax, mpCurrent: 0, mpMax: 0, activeEffects: emptyEffects(), onHitEffects: [...(creature.statusEffects ?? []), ...itemEffects], stats: { physicalDamage: creature.physicalDamage + (itemModifiers.physicalDamage ?? 0), magicalDamage: (creature.magicalDamage ?? 0) + (itemModifiers.magicalDamage ?? 0), physicalDefense: creature.physicalDefense + (itemModifiers.physicalDefense ?? 0), magicalDefense: creature.magicalDefense + (itemModifiers.magicalDefense ?? 0), criticalChance: (highRarity ? 12 : creature.rarity === "rare" ? 7 : 4) + (itemModifiers.criticalChance ?? 0), dodgeChance: (creature.rarity === "rare" ? 5 : 2) + (itemModifiers.dodgeChance ?? 0), bleedChance: itemModifiers.bleedChance ?? 0, burnChance: itemModifiers.burnChance ?? 0, poisonChance: itemModifiers.poisonChance ?? 0, blindChance: itemModifiers.blindChance ?? 0, bleedResistance: itemModifiers.bleedResistance ?? 0, burnResistance: itemModifiers.burnResistance ?? 0, poisonResistance: itemModifiers.poisonResistance ?? 0, blindResistance: itemModifiers.blindResistance ?? 0 } };
  });
...
export function resolveHuntTurn(state: HuntBattleState, ability: AbilityDefinition, targetId?: string): HuntBattleState {
  ...
  for (const enemy of enemies.filter((entry) => entry.hpCurrent > 0)) {
    const enemyDamageKind = enemy.stats.magicalDamage > enemy.stats.physicalDamage ? "magical" : "physical";
    const enemyRawDamage = enemyDamageKind === "magical" ? enemy.stats.magicalDamage : enemy.stats.physicalDamage;
    const counter = attack({ attacker: enemy, defender: player, rawDamage: enemyRawDamage, kind: enemyDamageKind, effects: enemy.onHitEffects, sourceName: enemy.name, turn: state.turn, logs });
